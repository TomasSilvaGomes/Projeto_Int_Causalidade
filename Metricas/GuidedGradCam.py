...
def guided_gradcam(model, x, target, layer):
    """Implementação manual do Guided Grad-CAM."""
    model.eval()
    activations = []
    gradients = []
...
# === AVALIAÇÃO FINAL E VISUALIZAÇÃO OTIMIZADA ================================
...
def evaluate_guided_gradcam():
    ...
    results = {
        ...
        "ROAD": ROAD(model, x_batch, y_batch, guided_gradcam, 
                     n_steps_metric=50, 
    ...
    label_to_show = 0
    fig.suptitle(f"Análise Visual das Métricas: Guided Grad-CAM (Label {label_to_show})", fontsize=22, fontweight='bold', y=0.98)
    ...
    axes[row, 1].set_title("Foco de Atribuição", fontsize=12) # Texto simplificado
    ...
    axes[row, 2].set_title("Áreas Removidas (Top 20%)", fontsize=12) # Texto direto
    ...
    axes[row, 3].set_xlabel("Maior queda de confiança\nimplica melhor Selectivity.", fontsize=11) # Afirmação
    ...
    axes[row, 2].set_title("Saliência sob Ruído", fontsize=12) # Texto direto
    ...
    axes[row, 3].set_xlabel("Menor diferença visual\nimplica melhor Continuity.", fontsize=11) # Afirmação
    ...
    axes[row, 2].set_title("Remoção Cumulativa (50%)", fontsize=12) # Texto direto
    ...
    axes[row, 3].set_xlabel("Média da queda de confiança\nao longo da curva de remoção.", fontsize=11) # Afirmação
    ...
    plt.tight_layout()
    plot_path = os.path.join(OUTPUT_DIR, f"GuidedGradCAM_Avaliacao_Label{label_to_show}.png")
