...
def saliency(model, x, target, **kwargs):
    """
    Implementação do Saliency Map (Gradiente Simples).
    Retorna o gradiente da saída em relação à entrada.
    Aceita **kwargs para compatibilidade, mas não os utiliza.
    """
    model.eval()
    # Garante que x requer gradiente e é uma folha no grafo
    ...
    # Backpropagate para obter o gradiente em relação a x
...
def evaluate_saliency():
    print("\nIniciando avaliação com Saliency (Gradient)...")
    ...
    label_to_show = 0
    fig.suptitle(f"Análise Visual das Métricas: Saliency (Label {label_to_show})", fontsize=22, fontweight='bold', y=0.98)
    ...
    attr_noisy = saliency(model, x_noisy, y_sample)
    ...
    plt.tight_layout()
    plot_path = os.path.join(OUTPUT_DIR, f"Saliency_Avaliacao_Label{label_to_show}.png")
